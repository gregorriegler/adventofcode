...
fun <T> List<T>.commonsCombinations(k: Int): Sequence<List<T>> =
    combinationsIterator(this.size, k)
        .asSequence()
        .map { it -> it.map { this[it] } }

fun <T> List<T>.combinations(k: Int): Sequence<List<T>> = recursiveCombinationGenerator(this, k)

fun <T> recursiveCombinationGenerator(
    input: List<T>,
    k: Int,
    building: MutableList<T> = mutableListOf()
...
fun List<Int>.hasSumEqualTo(expected: Int): Boolean = this.sum() == expected

fun Iterable<Int>.product(): Int = reduce { acc, it -> acc * it }
